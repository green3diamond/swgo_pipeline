...
from modules import TimestepEmbedder, ResNetBlock, RMSNorm, SwiGLU, SelfAttentionBlock
...
class ComponentWiseMSELoss(nn.Module):
    """
    Computes the sum of mean squared errors for each feature component,
    with special periodic wrapping for the 'phi' component (the last one).
    This ensures the training loss metric is consistent with the validation `get_pred_loss`.
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        # Feature-wise errors
        x_loss     = (diff[..., 0] ** 2).mean()
        y_loss     = (diff[..., 1] ** 2).mean()
        E_loss     = (diff[..., 2] ** 2).mean()
        theta_loss = (diff[..., 3] ** 2).mean()

        # φ wrapping to [-π, π) for the last component
        phi_diff   = torch.remainder(diff[..., 4] + torch.pi, 2 * torch.pi) - torch.pi
        phi_loss   = (phi_diff ** 2).mean()

        # The total loss is the sum of the per-component MSEs
...
class CFMDiagramModel(nn.Module):
    ...
    def __init__(
        ...
        dropout=0.1,
        attn_heads=8,
        attn_every=1,            # add a tiny self-attn block after every N ResNet blocks
        use_rmsnorm=True,
        ffn_mult=4,
    ):
        super().__init__()
        ...
        # Cross-attention to align noisy target with conditioning features
        ...
        # Residual + intermittent self-attention stack
        self.resnet_blocks = nn.ModuleList([ResNetBlock(model_dim, dropout=dropout) for _ in range(num_res_blocks)])
        self.self_attn_idx = set([i for i in range(num_res_blocks) if (i + 1) % max(attn_every, 1) == 0])
        self.self_attn_blocks = nn.ModuleDict({
            f"attn_{i}": SelfAttentionBlock(model_dim, num_heads=attn_heads, dropout=dropout, use_rmsnorm=use_rmsnorm, ffn_mult=ffn_mult)
            for i in self.self_attn_idx
        })

        # Final gated head (SwiGLU) for better expressivity
        self.final_mlp = nn.Sequential(
            nn.Linear(model_dim, model_dim * ffn_mult),
            SwiGLU(model_dim * ffn_mult, model_dim * ffn_mult),  # gated nonlinearity
            nn.Dropout(dropout),
    ...
    def forward(self, x_t: torch.Tensor, cond: torch.Tensor, t: torch.Tensor):
        ...
        # Condition features: support (B, 4) or (B, M, 4)
        cond_emb = self.cond_mlp(cond)

        # Noisy target features
        h = self.noisy_target_mlp(x_t)

        # Cross-attention: Q=h, K/V=cond_emb
        q = h.unsqueeze(1)  # (B, 1, H)
        if cond_emb.dim() == 3:
            k = v = cond_emb                      # (B, M, H)
        else:
            k = v = cond_emb.unsqueeze(1)         # (B, 1, H)
        attn_output, _ = self.cross_attention(query=q, key=k, value=v)
        h = self.attention_norm(h + attn_output.squeeze(1))  # residual
        ...
        # Residual stack with intermittent self-attention
        for i, block in enumerate(self.resnet_blocks):
            h = block(h)
            if i in self.self_attn_idx:
                h = self.self_attn_blocks[f"attn_{i}"](h)
...
class CFMLightningModule(pl.LightningModule):
    def __init__(
        ...
        optimizer="adamw",
        use_scheduler=False,
        scheduler_type="none",        # NEW: "none" | "cosine" | "onecycle"
        sample_steps=30,              # NEW: fewer steps than 50 for faster sampling
        attn_heads=8,                 # pass-through to model
        attn_every=1,                 # pass-through to model
        use_rmsnorm=True,
        ffn_mult=4,
    ):
        super().__init__()
        ...
        # Use component-wise MSE loss with phi wrapping
        self.criterion = ComponentWiseMSELoss()
        rank_zero_info("Using Component-wise MSE loss (with phi wrapping).")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred = self.dpm.sample(
            ...
            mask=None,
            global_data=None,
            steps=int(self.hparams.sample_steps),   # was 50
            method="multistep",
            skip_type="time_uniform_flow",
            show_progress=False,
        )

        loss_dict = self.get_pred_loss(pred, x1)

        # Log total prediction loss
        self.log("val_pred_loss", loss_dict["total_loss"], on_epoch=True, prog_bar=True, logger=True, sync_dist=True)

        # Log individual feature losses for detailed analysis
        self.log("val_x_loss",     loss_dict["x_loss"],     on_epoch=True, logger=True, sync_dist=True)
        self.log("val_y_loss",     loss_dict["y_loss"],     on_epoch=True, logger=True, sync_dist=True)
        self.log("val_E_loss",     loss_dict["E_loss"],     on_epoch=True, logger=True, sync_dist=True)
        self.log("val_theta_loss", loss_dict["theta_loss"], on_epoch=True, logger=True, sync_dist=True)
        self.log("val_phi_loss",   loss_dict["phi_loss"],   on_epoch=True, logger=True, sync_dist=True)

        # Log the training-style loss on the validation set
        self.log("val_loss",      loss,      on_epoch=True, prog_bar=True, logger=True, sync_dist=True)

        return {"val_loss": loss, "val_pred_loss": loss_dict["total_loss"]}
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        samples = self.dpm.sample(
            ...
            mask=None,
            global_data=None,
            steps=int(self.hparams.sample_steps),
            method="multistep",
            skip_type="time_uniform_flow",
        )
        return samples
    ...
    @torch.no_grad()
    def get_pred_loss(self, pred: torch.Tensor, true: torch.Tensor):
        """
        Compute per-feature MSE between pred and true, with periodic wrapping for phi.
        Works for shapes (B, 5) or (B, N, 5) by averaging over all elements.
        Returns a dict with individual components and total for logging.
        """
        # Ensure shape is (B, N, 5) for consistent processing
        if pred.dim() == 2:
        ...
        # Feature-wise errors (mean over batch and N dimensions)
        x_loss     = (diff[..., 0] ** 2).mean()
        y_loss     = (diff[..., 1] ** 2).mean()
        E_loss     = (diff[..., 2] ** 2).mean()
        theta_loss = (diff[..., 3] ** 2).mean()
    ...
    def configure_optimizers(self):
        ...
        sched_type = str(self.hparams.scheduler_type).lower()
        if sched_type in ("none", ""):
            rank_zero_info("No LR scheduler.")
            return optimizer
        ...
        if sched_type == "cosine":
            ...
            scheduler = CosineAnnealingWarmupRestarts(optimizer, **default_scheduler_dict)
            return {"optimizer": optimizer, "lr_scheduler": scheduler}

        if sched_type == "onecycle":
            # Lightning will call this after dataloaders are set, so trainer is available
            total_steps = self.trainer.estimated_stepping_batches
            rank_zero_info(f"Using OneCycleLR for {total_steps} steps.")
            scheduler = torch.optim.lr_scheduler.OneCycleLR(
                optimizer,
                max_lr=base_lr * 8,               # aggressive peak; tune if needed
            ...
            return {
                ...
                "lr_scheduler": {
                    "scheduler": scheduler,
                    "interval": "step",            # OneCycle should step every iteration
                }
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scheduler_type", type=str, default="none", choices=["none", "cosine", "onecycle"],
                        help="Learning rate scheduler to use.")
    parser.add_argument("--sample_steps", type=int, default=30, help="DPM sampling steps during val/predict.")
    parser.add_argument("--attn_heads", type=int, default=8, help="Number of heads for self/cross attention.")
    parser.add_argument("--attn_every", type=int, default=2, help="Insert a self-attn block after every N ResNet blocks.")
    parser.add_argument("--use_rmsnorm", action="store_true", help="Use RMSNorm inside self-attention blocks.")
    parser.add_argument("--ffn_mult", type=int, default=4, help="Expansion factor for gated FFN.")
    ...
    if args.save_ckpt:
        checkpoint_callback = ModelCheckpoint(
            dirpath=output_dir,
            filename='{epoch:03d}-{val_pred_loss:.4f}-{val_loss:.4f}',
        ...
        if is_main_process:
            print(f"INFO: Model checkpoints will be saved to '{output_dir}', monitored by val_pred_loss.")
    else:
        if is_main_process:
