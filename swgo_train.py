...
class SWGODataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        try:
            ...
            print(f"Data loaded: {len(self.train_dataset)} train, {len(self.val_dataset)} val/test samples")
        except FileNotFoundError as e:
            print(f"Error: {e}. Please ensure '{self.train_data_path}' and '{self.val_data_path}' exist.")
            exit()
        except ValueError:
            print(f"Error loading data. Ensure .pt files contain a tuple of (inputs, labels).")
            exit()
...
class CFMLightningModule(pl.LightningModule):
    def __init__(self, learning_rate=3e-5, num_res_blocks=3, flow_matcher_type="target",
                 dropout=0.1, optimizer="adamw", use_scheduler=False):
        super().__init__()
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        cond, _ = batch
        num_steps = 100
        # Predict target in (x0, y0, E, theta, phi)
        x = torch.randn(cond.size(0), 5, device=self.device)
        time_steps = torch.linspace(0, 1, num_steps + 1, device=self.device)
        step_size = time_steps[1] - time_steps[0]
        for i in range(num_steps):
            t = time_steps[i]
            v_pred = self(x, cond, t.repeat(cond.size(0)))
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.optimizer == "lion":
            print(f"Using Lion optimizer with learning_rate={base_lr}")
            optimizer = Lion(self.parameters(), lr=base_lr)
        elif self.hparams.optimizer == "adamw":
            print(f"Using AdamW optimizer with learning_rate={base_lr}")
