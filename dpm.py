...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model = lambda x, timestep, truth, mask, global_data: model_fn(
            x, timestep.expand(x.shape[0]), truth, mask, global_data
        )
    ...
    def noise_prediction_fn(self, x, t, truth, mask, global_data):
        """
        Return the noise prediction model.
        """
        return self.model(
            x, timestep=t, truth=truth, mask=mask, global_data=global_data
        )

    def data_prediction_fn(self, x, t, truth, mask, global_data):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, t, truth, mask, global_data)
        alpha_t, sigma_t = (
    ...
    def model_fn(self, x, t, truth, mask, global_data):
        """
        Convert the model to the noise prediction model or the data prediction model.
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, truth, mask, global_data)
        else:
            return self.noise_prediction_fn(x, t, truth, mask, global_data)
    ...
    def denoise_to_zero_fn(self, x, s, truth, mask, global_data):
        """
        Denoise at the final step, which is equivalent to solve the ODE from lambda_s to infty by first-order discretization.
        """
        return self.data_prediction_fn(x, s, truth, mask, global_data)

    def dpm_solver_first_update(
        self, x, s, t, truth, mask, global_data, model_s=None, return_intermediate=False
    ):
        """
        ...
        if self.algorithm_type == "dpmsolver++":
            phi_1 = torch.expm1(-h)
            if model_s is None:
                model_s = self.model_fn(x, s, truth, mask, global_data)
    ...
    def singlestep_dpm_solver_second_update(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s1 = self.model_fn(x_s1, s1, truth, mask, global_data)
    ...
    def singlestep_dpm_solver_third_update(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s2 = self.model_fn(x_s2, s2, truth, mask, global_data)
            if solver_type == "dpmsolver":
                x_t = (
                    (sigma_t / sigma_s) * x
                    - (alpha_t * phi_1) * model_s
    ...
    def dpm_solver_adaptive(
        ...
        if order == 2:
            ...
            lower_update = lambda x, s, t: self.dpm_solver_first_update(
                x, s, t, truth, mask, global_data, return_intermediate=True
            )
            higher_update = (
    ...
    def sample(
        ...
        with torch.no_grad():
            if method == "adaptive":
                ...
                t = timesteps[step]
                t_prev_list = [t]
                model_prev_list = [self.model_fn(x, t, truth, mask, global_data)]
                if self.correcting_xt_fn is not None:
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(
                        self.model_fn(x, t, truth, mask, global_data)
                    )
                    # update progress bar
                    self.update_progress(step + 1, len(timesteps))
                # Compute the remaining values by `order`-th order multistep DPM-Solver.
                for step in tqdm(range(order, steps + 1), disable=not show_progress):
                    t = timesteps[step]
                ...
                for step, order in tqdm(enumerate(orders), total=len(orders), disable=not show_progress):
            ...
            if denoise_to_zero:
                ...
                x = self.denoise_to_zero_fn(x, t, truth, mask, global_data)
                if self.correcting_xt_fn is not None:
