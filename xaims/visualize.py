...
def plot_losses(train_losses, val_losses=None, 
    ...
    Plot training and optional validation losses in two subplots:
      ...
      - Right: log-log scale (with safe shifting for non-positive values)
    ...
    shift = -min_value + 1e-8 if min_value <= 0 else 0  # ensure all values > 0

    ax_log.plot(train_losses + shift, color='black', label='Train', zorder=10)
    if val_losses is not None:
        ax_log.plot(val_losses + shift, color='red', label='Validation')
    
    ax_log.set_xscale('log')
    ax_log.set_yscale('log')
    ax_log.set_title("Log-Log Scale")
    ax_log.set_xlabel(xlabel)
    ax_log.set_ylabel(ylabel + (f" (shift +{shift:.2e})" if shift > 0 else ""))
    ax_log.legend()
