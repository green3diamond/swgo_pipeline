...
def split_loaders(
    ...
    """
    Split (X, y) into train/val/test TensorDatasets and DataLoaders

    Args:
        X (torch.Tensor): Input features
        y (torch.Tensor): Labels
        frac_train (float): Fraction of data for training
        frac_val (float): Fraction of data for validation
        batch_size (int): Batch size for DataLoaders
        shuffle (bool): Whether to shuffle the train set
