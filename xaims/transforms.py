...
class ZScoreTransform(nn.Module):
    ...
    Tracks constant features and restores them in reverse.
    """
    def __init__(self, mean: torch.Tensor, std: torch.Tensor, eps: float = 1e-6):
        ...
        valid_mask = (std.view(-1) > eps)
        self.register_buffer("valid_mask", valid_mask)

        # Keep only valid features
        mean_valid = mean.view(-1)[valid_mask]
        std_valid  = std.view(-1)[valid_mask]

        # Store mean/std only for valid features
        self.register_buffer("mean", mean_valid.view(1, -1))
        self.register_buffer("std",  std_valid.view(1, -1))

        # Store constants to restore later
        const_means = mean.view(-1)[~valid_mask]
        self.register_buffer("const_means", const_means)

    def forward(self, x: torch.Tensor, return_logdet: bool = False):
        # Select only variable features
        x_valid = x[:, self.valid_mask]

        normed = (x_valid - self.mean.to(x.device)) / (self.std.to(x.device) + self.eps)

        if return_logdet:
            logdet = -torch.sum(torch.log(self.std + self.eps), dim=-1)  # per sample
            return normed, logdet
        return normed

    def reverse(self, z: torch.Tensor, return_logdet: bool = False):
        # Reconstruct valid features
        x_valid = z * (self.std + self.eps) + self.mean

        # Reinsert constant features
        B = z.size(0)
        full_dim = self.valid_mask.numel()
        x_full = z.new_zeros((B, full_dim))

        # Fill variable features
        x_full[:, self.valid_mask] = x_valid
        # Fill constant features with stored means
        x_full[:, ~self.valid_mask] = self.const_means.to(z.device)

        if return_logdet:
            logdet = torch.sum(torch.log(self.std + self.eps), dim=-1)
